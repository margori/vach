...
<div class="report-technical row">
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/01-frontpage.png', [
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/02-VACH.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    <div class="col-lg-12 text-center">
        ...
        <h3>
            <?= Yii::t('report', 'Report') ?> <?= Yii::$app->formatter->asDate(
     'now'
 ) ?>
        </h3>
    </div>
    <div class="col-lg-12">
        <h4>
            <?= Yii::t('team', 'Company') ?>: <?= Html::label(
    $team->company->name
) ?>
            <br/>
            <?= Yii::t('team', 'Team') ?>: <?= Html::label($team->name) ?>
            <br/>
            <?= Yii::t('user', 'Coach') ?>: <?= Html::label(
    $team->coach->fullname
) ?>
            <br/>
            <?= Yii::t('team', 'Sponsor') ?>: <?= Html::label(
    $team->sponsor->fullname
) ?>
            <br/>
    ...
    <div class="col-lg-12">
        ...
        <h3>
            <ol>
                ...
                <li><?= Yii::t('report', 'Summary Stage 1') ?></li>
                <li><?= Yii::t(
                    'report',
                    'Summary Stage 2 and Conclusions'
                ) ?></li>
            </ol>
        </h3>
    </div>
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/03-introduction.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/04-fundaments.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/05-team.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/06-individuals.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    <div style="page-break-after: always"> </div>
    <div class="col-lg-12">
        <?php foreach ($team->report->individualReports as $individualReport) {
            if ($individualReport->teamMember->active) {

                $projectedGroupWheel = Wheel::getProjectedGroupWheel(
                    $team->id,
                    $individualReport->person_id
                );
                $projectedOrganizationalWheel = Wheel::getProjectedOrganizationalWheel(
                    $team->id,
                    $individualReport->person_id
                );
                $reflectedGroupWheel = Wheel::getReflectedGroupWheel(
                    $team->id,
                    $individualReport->person_id
                );
                $reflectedOrganizationalWheel = Wheel::getReflectedOrganizationalWheel(
                    $team->id,
                    $individualReport->person_id
                );
                ...
                $groupCompetences = Wheel::getMemberCompetences(
                    $team->id,
                    $individualReport->person_id,
                    Wheel::TYPE_GROUP
                );
                $organizationalCompetences = Wheel::getMemberCompetences(
                    $team->id,
                    $individualReport->person_id,
                    Wheel::TYPE_ORGANIZATIONAL
                );
                $groupEmergents = Wheel::getMemberEmergents(
                    $team->id,
                    $individualReport->person_id,
                    Wheel::TYPE_GROUP
                );
                $organizationalEmergents = Wheel::getMemberEmergents(
                    $team->id,
                    $individualReport->person_id,
                    Wheel::TYPE_ORGANIZATIONAL
                );
                ...
                <div style="page-break-after: always"> </div>
                <?php if ($individualReport->performance != '') {
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/07-summary.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
    ...
    <div class="jumbotron">
        <p>
            <?= Html::img('@web/images/brands/08-plan.png', [
                'class' => 'image-responsive',
            ]) ?>
        </p>
    </div>
